...
async function sendErrorLog(bot, error, type) {
  try {
    if (
      error.message?.includes("Eksik Erişim") ||
      error.message?.includes("Bilinmeyen Mesaj") ||
      error.message?.includes("Eksik İzinler")
    ) return;
...
function formatDuration(millisec) {
  if (!millisec || !Number(millisec)) return "0 saniye";
...
function modifyQueue(interaction) {
  ...
  if (!memberChannelId) {
    return interaction.client.say.errorMessage(interaction, "Önce bir ses kanalına katılmanız gerekiyor!");
  }

  if (memberChannelId !== botChannelId) {
    return interaction.client.say.warnMessage(interaction, "Benimle aynı ses kanalında olmalısın!");
  }
